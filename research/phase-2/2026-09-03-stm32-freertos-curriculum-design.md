...
| **S-09** | Original 64 KB Linker Script | Repository Author | Course Source Code | `stm32f103c8tx_flash.ld` (2026) | Entire file | Original pedagogical linker script for the 64 KB C8 target, retaining init-array sections via `KEEP` and boundary symbols for the `-nostartfiles` newlib-nano runtime contract. No vendor linker-script text is copied; repository-wide licensing is a separate governance decision and is not invented here. ST's Ac6 template remains read-only reference only. |
| **S-10** | Arm GNU Toolchain 13.3.rel1 | Arm Limited | Toolchain Distribution | 13.3.rel1 (Jul 2024) | GCC 13.3.1, Binutils 2.42, GDB 14.2, Newlib 4.4.0 | Pinned compiler/linker baseline with `-nostartfiles` startfile suppression and course-owned minimal `_init/_fini` runtime glue. The distribution contains components under multiple upstream licenses; implementation notes must preserve the release package's license notices rather than collapse them into one label. |
