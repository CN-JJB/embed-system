...
|---|---|---|---|---|---|
| **QEMU ARM `virt` (Canonical Baseline)** | **Cortex-A7 (32-bit ARMv7-A)** | `arm-none-linux-gnueabihf-` (canonical; distro alternate documented separately) | Direct 32-bit register/pointer continuity with Phase 2; fast compilation; transparent memory map; standard PL011 and GICv2; dynamic DTB extraction; low host RAM usage | Default CPU is Cortex-A15 unless `-cpu cortex-a7` is explicit | **SELECTED PRIMARY BASELINE** |
...
| **L03** | **BusyBox Multi-Call Binary** | **1.36.1** (tag `1_36_1` [lightweight tag, commit `1a64f6a20aaf6ea4dbba68bbfa8cc1ab7e5c57c4`], released 2023-05-19) | GPL-2.0-only | `https://git.busybox.net/busybox/` | Minimal userspace environment; `/sbin/init`, coreutils, shell (`sh`), multi-call dispatcher; chosen for rock-solid stability |
