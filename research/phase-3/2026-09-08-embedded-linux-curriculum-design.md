...
> Toolchain Baseline: **Arm GNU Toolchain 13.3.rel1** (`arm-linux-gnueabihf-gcc` 13.3.1 20240614, Binutils 2.42, Glibc 2.39 / Musl 1.2.5)  
> Upstream Software Baselines: **Linux Kernel 6.18.y LTS**, **QEMU 11.1.1**, **BusyBox 1.36.1**, **Buildroot 2026.05.2**, **DTC v1.7.0**  
...
5. **Device Tree First Pass (L2–L3 DT / L3 resource representation)**:
   Decompile compiled Device Tree Blobs (`.dtb`) into human-readable source (`.dts`) using `dtc`; explain tree nodes, property types (strings, 32-bit cells, byte arrays, phandles), `compatible` driver-matching strings, `reg` address/length mappings, and `interrupts` interrupt controller descriptors; modify device properties (e.g. toggling `status = "disabled"`); verify device registration at runtime in `/sys/firmware/devicetree/base`.
6. **Shallow Buildroot Automation (L2–L3 Buildroot / L3 reproducibility)**:
   Configure and execute an automated Buildroot pipeline to produce an identical kernel, rootfs, and image; customize userspace via rootfs overlays (`BR2_ROOTFS_OVERLAY`); audit generated build artifacts in `output/images/`, `output/target/`, and `output/build/`; explain what Buildroot automates versus manual assembly; diagnose package build stamp and configuration drift issues.
7. **Architecture Spine: Privilege, Traps, MMU & Memory Attributes (L2–L3 architecture spine / L3 VM)**:
   Trace the transition between unprivileged user execution (ARM PL0 / EL0) and privileged kernel execution (ARM PL1 / EL1); explain system call entry via the `svc` trap instruction; inspect process memory mappings in `/proc/<pid>/maps`; explain two-level virtual memory address translation on ARMv7-A (Translation Table Base Register `TTBR0`/`TTBR1`, First-Level Page Directory, Second-Level Page Table, 4 KB pages); explain the role of the Translation Lookaside Buffer (TLB) and why context switches require TLB flushes (or ASID tagging); contrast Normal Memory (cacheable, speculative, out-of-order) with Device MMIO (non-cacheable, strictly ordered, side effects) and explain the role of memory barriers (`DSB`, `ISB`, `DMB`).
8. **Evidence-Driven Boot-Chain Fault Isolation (L3 $\rightarrow$ L4-local on defined fault classes)**:
   Given an unfamiliar system boot failure, independently execute the 7-step diagnostic loop:
   ...
   Collect discriminating evidence from serial logs, GDB registers, `readelf`, `file`, `/proc`, and sysfs without guessing or relying on AI generation.
...
graph TD
    ...
    subgraph Phase 3 Embedded Linux Boot-Chain
        ...
        P3_KERN[P3-M02: Kernel Source, Kconfig, Build & Image Artifacts]
        ...
        P3_GATE[P3-GATE: Phase 3 Final Gate Assessment]
    end
    ...
    P2_BOOT -->|Linker LMA to VMA transfer -> Kernel decompression| P3_KERN
    ...
    P2_SCHED -->|Thread/Handler privilege -> User/Kernel PL0/PL1| P3_ARCH
...
3. **Phase 2 Bare-Metal Startup $\longrightarrow$ Phase 3 Kernel Decompression & Placement**:
   In P2-M01, learners wrote assembly code to copy initialized data from Flash (LMA) to SRAM (VMA). In P3-M02, learners observe the exact same pattern on an operating-system scale: the bootloader/QEMU places the compressed kernel (`zImage`) in RAM; the self-extracting header in `arch/arm/boot/compressed/head.S` decompress the kernel into executable memory (`vmlinux`), sets up initial identity page tables, and jumps to `stext` (`arch/arm/kernel/head.S`).
...
Phase 3 introduces the architectural foundation of general-purpose operating systems on the **ARMv7-A** profile (Cortex-A series). It focuses on the mechanisms required to understand Linux execution, isolation, and device access:
...
|                                                                                       |
|   +-------------------------------------------------------------------------------+   |
|   | USER SPACE (Unprivileged - PL0 / EL0)                                         |   |
|   | - Applications, Shell, Daemons, BusyBox utilities                            |   |
|   | - Virtual Address Space: 0x00000000 -> 0xBFFFFFFF (3 GB User Virtual Memory) |   |
|   | - Restricted CP15 access, restricted instructions, unprivileged mode (USR)    |   |
|   +-------------------------------------------------------------------------------+   |
|                                          |                                            |
|                                          | System Call / Trap (`svc #0` instruction)  |
|                                          v                                            |
|   +-------------------------------------------------------------------------------+   |
|   | KERNEL SPACE (Privileged - PL1 / EL1)                                         |   |
|   | - Linux Kernel Core, VFS, Scheduler, Device Drivers, Page Table Management   |   |
|   | - Virtual Address Space: 0xC0000000 -> 0xFFFFFFFF (1 GB Kernel Virtual Space)|   |
|   | - Supervisor Mode (SVC), System Mode (SYS), Abort (ABT), IRQ / FIQ Modes     |   |
...
On ARMv7-A, execution privileges are divided into Privilege Levels:
- **PL0 (User / Unprivileged)**: Code runs in User mode (`USR`). Access to hardware control registers (CP15) is trapped; direct access to physical memory outside mapped pages generates a Data Abort exception.
...
1. The userspace C library places the system call number in register `r7` (on ARM EABI) and arguments in `r0`–`r5`.
...
## 3.2 Virtual Memory & Two-Level Page Table Translation

On ARMv7-A Virtual Memory System Architecture (VMSA), a 32-bit Virtual Address (VA) is translated to a Physical Address (PA) via a two-level page table hierarchy:
...
- **Translation Table Base Registers (`TTBR0` and `TTBR1`)**:
  - `TTBR0` holds the physical base address of the Level 1 translation table for user space (`0x00000000` to `0xBFFFFFFF`). Each process has its own unique Level 1 table, swapped during context switches (`switch_mm()`).
...
- **Level 1 Page Table (PGD)**: 4096 entries of 4 bytes each (16 KB total). Indexed by VA bits `[31:20]`. Each entry represents 1 MB of virtual address space. It can either map a 1 MB Section directly or point to a Level 2 Page Table.
...
## 3.3 TLB & Cache Organization

- **Translation Lookaside Buffer (TLB)**:
  Hardware cache that stores recent VA $\rightarrow$ PA translations to avoid traversing memory-resident page tables on every instruction fetch or data access.
  - *Context Switch Consequence*: When the kernel switches between two userspace processes with different address spaces, stale TLB entries must not be used. On processors without hardware Address Space Identifiers (ASID), the kernel must invalidate the entire unified TLB (`flush_tlb_all()`). On modern ARMv7-A with ASID support, translations are tagged with an 8-bit ASID, eliminating full TLB flushes on context switch.
- **L1 Cache Hierarchy (Harvard Architecture)**:
  - Separate **L1 Instruction Cache (I-Cache)** and **L1 Data Cache (D-Cache)**, typically 32 KB each with 32-byte or 64-byte line sizes.
  - *Write Policy*: Normal memory is configured as **Write-Back, Write-Allocate**. Writes to memory update the D-cache line and mark it *dirty*; data is written back to main DRAM only when the line is evicted.
- **Memory Types: Normal Memory vs Device MMIO**:
  - **Normal Memory**: Used for program code, stacks, heaps, and OS buffers. Supports speculative prefetching, out-of-order execution, instruction caching, and data write-buffering.
  - **Device Memory (MMIO)**: Used for peripheral registers (UART, timers, interrupt controller). **Strictly non-cacheable**, non-speculative, and preserves access size and count. Reads and writes have physical side effects.
  - **Memory Barriers**:
    - `DMB` (Data Memory Barrier): Ensures that all previous explicit memory transactions complete before subsequent memory transactions.
    - `DSB` (Data Synchronization Barrier): Halts execution until all outstanding memory transactions (including write buffers) complete.
    - `ISB` (Instruction Synchronization Barrier): Flushes the CPU instruction pipeline, forcing subsequent instructions to be re-fetched from cache or memory (essential after modifying page tables or CP15 control registers).
...
A core mandate of Issue #27 is that Phase 3 must be **QEMU-first** and require **no physical board purchase**. The design evaluated four candidate architectures:

| Platform Candidate | Processor Profile | Target Tuple | Strengths | Weaknesses | Decision |
|---|---|---|---|---|---|
| **QEMU ARM `virt` (Canonical Baseline)** | **Cortex-A7 (32-bit ARMv7-A)** | `arm-linux-gnueabihf` | Direct 32-bit register/pointer continuity with Phase 2; fast compilation; transparent memory map; standard PL011 and GICv2; dynamic DTB extraction; low host RAM usage | 32-bit architecture (older than modern 64-bit phones) | **SELECTED PRIMARY BASELINE** |
...
**Selection Rationale**:
The **QEMU `-M virt` machine with Cortex-A7** is selected as the canonical baseline because:
1. **32-bit Continuity**: Memory addresses, pointers, and registers are 32-bit (`uint32_t`), directly reinforcing the pointer arithmetic, struct alignment, and memory layouts mastered in Phase 1 and Phase 2.
2. **Upstream Alignment**: The `virt` machine is actively maintained by the upstream QEMU and Linux kernel communities. It is not tied to an obsolete vendor board.
3. **Transparent Device Model**: Peripherals are standard ARM primitives: ARM Generic Interrupt Controller (GICv2), ARM PrimeCell PL011 UART, and ARM Generic Timers.
4. **Dynamic Device Tree Extraction**: QEMU dynamically generates the exact Device Tree matching the configured hardware, allowing learners to dump it directly via `-machine virt,dumpdtb=virt.dtb` and decompile it with `dtc`.
...
Learners utilize two distinct QEMU execution contracts:
...
qemu-system-arm \
    ...
    -append "console=ttyAMA0 earlycon=pl011,0x09000000 rdinit=/sbin/init panic=1"
```
...
qemu-system-arm \
    ...
    -append "console=ttyAMA0 earlycon=pl011,0x09000000 root=/dev/vda rw rootwait panic=1"
```
...
All source and toolchain baselines are pinned to official upstream releases checked as of **2026-09-08**:

| ID | Component / Document | Canonical Baseline Version / Pin | License | Upstream Source Repository / URL | Pedagogical Role & Scope |
|---|---|---|---|---|---|
| **L01** | **Linux Kernel** | **6.18.y LTS** (exact commit snapshot pinned at lab freeze) | GPL-2.0-only | `git://git.kernel.org/pub/scm/linux/kernel/git/stable/linux.git` | Primary operating system kernel; source tree navigation, Kconfig, boot initialization in `init/main.c` |
| **L02** | **QEMU System Emulator** | **11.1.1** (or 9.2+ series verified compatible) | GPL-2.0 | `https://gitlab.com/qemu-project/qemu.git` | Canonical hardware emulator; provides `-M virt` virtual machine platform and GDB stub |
| **L03** | **BusyBox Multi-Call Binary** | **1.36.1** / 1.37.0 | GPL-2.0-only | `https://git.busybox.net/busybox/` | Minimal userspace environment; `/sbin/init`, coreutils, shell (`sh`), multi-call dispatcher |
| **L04** | **Buildroot System** | **2026.05.2** / 2026.02 LTS | GPL-2.0-or-later | `https://gitlab.com/buildroot.org/buildroot.git` | Automated embedded build system; toolchain, rootfs generation, rootfs overlay, reproducible images |
| **L05** | **Device Tree Compiler (DTC)** | **v1.7.0** / v1.7.2 | GPL-2.0-or-later / BSD-2-Clause | `https://git.kernel.org/pub/scm/utils/dtc/dtc.git` | Device Tree decompiler and compiler; DTS $\leftrightarrow$ DTB conversion and validation |
| **L06** | **Arm GNU Toolchain** | **13.3.rel1** (`arm-linux-gnueabihf`) | GPL-3.0 / LGPL-2.1 | `https://developer.arm.com/downloads/-/arm-gnu-toolchain-downloads` | Cross-compilation toolchain; GCC 13.3.1, Binutils 2.42, Glibc 2.39 / Musl 1.2.5 |
| **L07** | **Devicetree Specification** | **Release v0.4** | Creative Commons Attribution | `https://www.devicetree.org/specifications/` | Official specification for device tree syntax, standard properties (`compatible`, `reg`, `interrupts`) |
| **L08** | **ARM Architecture Reference** | **ARM DDI 0406C.d (Armv7-A/R)** | Proprietary / Arm Reference | Arm Infocenter / Developer Documentation | Authoritative specification for ARMv7-A privilege modes, VMSA MMU, CP15 registers, and barriers |
...
└── P3-M02: Linux Kernel Source Orientation, Kconfig, Build Flow & Image Artifacts (4.0 h MUST)
...
└── P3-GATE: Phase 3 Final Gate Assessment (AI-Free 4-Part Transfer Examination) (3.0 h MUST)
```
...
### 1. Context & Why Now
Before building a Linux kernel or root filesystem, the developer must master the cross-development environment. Unlike Phase 1 (where host GCC compiled for the host architecture) and Phase 2 (where `arm-none-eabi-gcc` targeted bare-metal Cortex-M with no operating system), Embedded Linux cross-compilation targets a specific architecture and operating system ABI (`arm-linux-gnueabihf`). Understanding target tuples, sysroots, dynamic loaders, and shared library linkages is mandatory to prevent and diagnose binary execution failures.
...
- Target tuple anatomy: `[arch]-[vendor]-[os]-[abi]` (e.g. `arm-unknown-linux-gnueabihf`).
...
- Inspect the output of `arm-linux-gnueabihf-gcc -print-sysroot` and locate the target `libc.so.6` and `ld-linux-armhf.so.3`.
...
### 7. Hands-On Labs
- **Lab 1.1**: Cross-compile a simple C program (`hello_target.c`) using `arm-linux-gnueabihf-gcc`. Inspect the resulting ELF header with `readelf -h hello_target` and identify `Machine: ARM`, `Class: ELF32`, `Data: 2's complement, little endian`, and `Flags: 0x5000400, Version5 EABI, hard-float ABI`.
...
### 8. Expected Evidence
- Verifiable terminal logs showing `readelf -h` and `readelf -l` output for both dynamic and static target binaries.
- Output of `arm-linux-gnueabihf-readelf -d hello_target` listing `(NEEDED) Shared library: [libc.so.6]`.
...
### 10. Deliberate Fault
Seed an executable compiled with `arm-linux-gnueabihf-gcc` into a minimal rootfs that only contains `musl` dynamic libraries (`/lib/ld-musl-armhf.so.1`). The learner must diagnose why invoking `./app` fails with `sh: ./app: not found` even though `ls -l ./app` confirms the file exists and is executable. Root cause: the kernel fails to locate the glibc interpreter `/lib/ld-linux-armhf.so.3`.
...
### 1. Context & Why Now
With cross-toolchain mechanics understood, the learner builds the core of the operating system. The Linux kernel is a massive codebase (~30 million LOC), but an embedded developer does not read it linearly. The developer must understand its directory architecture, configuration system (`Kconfig`), build system (`Kbuild`), and the relationship between generated image artifacts (`vmlinux`, `zImage`, `System.map`).
...
### 3. Mental Model
The Linux kernel uses an interactive configuration system. `Kconfig` files define hierarchical configuration options and dependencies. Running `make menuconfig` (or using a pre-existing `defconfig`) outputs a flat key-value file named `.config`. Kbuild reads `.config` and conditionally compiles only the selected source files. Compilation produces an uncompressed ELF binary with full debug symbols (`vmlinux`). To save bootloader load time and Flash space, architecture-specific post-processing compresses `vmlinux` using gzip/LZO/XZ and prepends an assembly decompressor stub, producing the self-extracting boot image (`zImage`).
...
- Configuration system: `Kconfig` syntax (`config`, `bool`, `tristate`, `depends on`, `select`, `default`), `.config` generation, `make defconfig`, `make menuconfig`.
- Build targets:
  ...
  - `zImage`: Self-extracting compressed bootable kernel image loaded by QEMU or U-Boot.
  - `System.map`: Symbol lookup table mapping kernel symbol names to virtual memory addresses.
- Cross-compilation variables: `ARCH=arm` and `CROSS_COMPILE=arm-linux-gnueabihf-`.
...
- Pinned upstream source: `init/main.c` (reading lines around `start_kernel()`).

### 6. Source-Reading Tasks
- Read `init/main.c` around `asmlinkage __visible void __init __no_sanitize_address start_kernel(void)`. Identify calls to `setup_arch()`, `trap_init()`, `mm_init()`, `sched_init()`, `init_IRQ()`, `time_init()`, and `rest_init()`.
- Read `arch/arm/boot/Makefile` to see how `zImage` is generated from `vmlinux` via `objcopy` and compression.
...
- **Lab 2.1**: Set up the kernel build environment:
  ...
  ```
  Inspect `.config` and verify that `CONFIG_ARM=y` and `CONFIG_PRINTK=y`.
- **Lab 2.2**: Compile the kernel:
  ```bash
  make -j$(nproc) zImage
  ```
  Inspect the generated artifacts: `vmlinux` in the root directory and `arch/arm/boot/zImage`. Use `ls -lh` to compare sizes (e.g. `vmlinux` ~30–50 MB vs `zImage` ~4–6 MB).
- **Lab 2.3**: Inspect kernel symbols using `nm vmlinux | grep start_kernel` and compare the address with the entry in `System.map`. Explain why the address is in the high virtual address range (e.g. `0xC0008000` or `0xC0100000`).
- **Lab 2.4**: Boot the compiled `zImage` in QEMU without a root filesystem:
  ```bash
  qemu-system-arm -M virt -cpu cortex-a7 -m 512M -nographic -kernel arch/arm/boot/zImage
  ```
...
- Terminal recording of QEMU executing `zImage` up to the VFS panic.
...
### 10. Deliberate Fault
Provide a `.config` where `CONFIG_PRINTK` is disabled (`# CONFIG_PRINTK is not set`). The learner runs the kernel in QEMU and observes completely dead serial output (black screen/silent terminal). The learner must diagnose the silence, inspect `.config`, identify the missing printk subsystem, restore `CONFIG_PRINTK=y`, and recompile.

### 11. Module Gate
AI-Free configuration test: From a clean kernel source tree, cross-compile a working `zImage` for ARM, extract the virtual address of `rest_init` from `vmlinux` using `readelf -s`, and demonstrate the kernel booting in QEMU up to the expected VFS root mount panic.
...
### 3. Mental Model
The Linux root filesystem is not just a collection of files; it is the environment in which the first userspace process runs. The kernel expects to find an executable at `/sbin/init` (or specified via `init=`). This executable becomes **PID 1**. PID 1 is responsible for initializing userspace: mounting virtual filesystems that provide a window into the kernel (`/proc` for processes, `/sys` for hardware devices, `/dev` for device nodes), configuring interfaces, running system scripts, and staying alive forever. If PID 1 exits, the entire operating system crashes.
...
- **Lab 3.6**: Boot QEMU with the kernel and initramfs:
  ```bash
  qemu-system-arm -M virt -cpu cortex-a7 -m 512M -nographic \
...
### 10. Deliberate Fault
Remove the executable bit from `/sbin/init` (`chmod -x rootfs/sbin/init`) and repackage the initramfs. Boot QEMU. Observe the kernel panic:
`Kernel panic - not syncing: Attempted to kill init! exitcode=0x0000000d` (or `-EACCES` error -13).
The learner must formulate hypotheses, inspect the error code, identify the missing executable permission, repair it with `chmod +x`, and verify clean boot.

### 11. Module Gate
AI-Free manual rootfs assembly: Given an empty directory and a cross-compiled BusyBox binary, construct a complete minimal rootfs with proper FHS layout, `/dev` nodes, and an init script that mounts `/proc` and `/sys`. Package it into an initramfs and demonstrate an interactive shell boot in QEMU.
...
- Inspect `drivers/tty/serial/amba-pl011.c` around `pl011_early_write()` to see the minimal polled MMIO register write loop (`while (readl(port->membase + UART01x_FR) & UART01x_FR_TXFF); writel(ch, port->membase + UART01x_DR);`).
...
- **Lab 4.1**: Boot QEMU with `earlycon` enabled:
  ...
  qemu-system-arm -M virt -cpu cortex-a7 -m 512M -nographic \
      -kernel zImage -initrd rootfs.cpio.gz \
      -append "earlycon=pl011,0x09000000 console=ttyAMA0 rdinit=/bin/sh"
  ```
...
- **Lab 4.3**: Capture the full boot log to a file on the host using QEMU redirection or pipe. Trace the log from `Booting Linux on physical CPU 0x0` through to the shell prompt. Annotate 5 key milestones: DRAM detection, GIC init, Timer init, Serial init, and rootfs mount.
...
### 9. Challenge
Given a QEMU boot that hangs silently after printing `Uncompressing Linux... done, booting the kernel.`, apply the 7-step diagnostic framework: articulate 3 hypotheses, enable `earlycon` via QEMU `-append`, capture the previously invisible early kernel crash, and identify the root cause.
...
The learner must independently diagnose all 4 faults from log evidence.

### 11. Module Gate
AI-Free diagnostic exam: The learner is presented with two blind QEMU instances exhibiting distinct boot failures. Within 30 minutes, collect log evidence, formulate hypotheses, identify exact root causes, and restore clean boots to an interactive shell.
...
### 10. Deliberate Fault
In `virt.dts`, change the `status` property of the primary serial node `pl011@9000000` to `status = "disabled";`. Recompile and boot. The learner observes that earlycon functions, but the standard serial driver refuses to probe the device, resulting in console freeze. The learner diagnoses the fault from DT evidence and repairs it.

### 11. Module Gate
AI-Free Device Tree test: Given an unfamiliar compiled DTB, use `dtc` to decompile it, identify a seeded address collision or disabled status property on a critical peripheral, repair the DTS source, recompile to DTB, and verify clean boot in QEMU.
...
- Buildroot directory architecture:
  - `configs/`: Predefined board configurations (`qemu_arm_vexpress_defconfig`).
...
### 7. Hands-On Labs
- **Lab 6.1**: Configure Buildroot for QEMU ARM:
  ```bash
  make qemu_arm_vexpress_defconfig
  ```
  Inspect `.config` and identify target architecture (`BR2_arm=y`, `BR2_cortex_a9=y` or `a7`).
...
### 10. Deliberate Fault
Modify an overlay file on the host after an initial build, run `make`, and observe that the changes do not appear in `output/images/rootfs.ext4`. The learner must diagnose why Buildroot did not re-trigger target assembly, locate the build stamp mechanism (`.stamp_target_installed`), run `make target-finalize`, and regenerate the image.

### 11. Module Gate
AI-Free build automation test: Given a clean Buildroot tree, configure an appliance image with a specified rootfs overlay, build the image, boot it in QEMU, and demonstrate automated execution of a custom diagnostic tool without interactive manual intervention.
...
### 3. Mental Model
Every userspace process runs in an isolated 3 GB virtual address space (`0x00000000` to `0xBFFFFFFF`), oblivious to other processes and physical DRAM boundaries. When the process accesses an address, the hardware Memory Management Unit (MMU) translates the Virtual Address (VA) to a Physical Address (PA) by walking two levels of page tables stored in RAM. The Translation Lookaside Buffer (TLB) caches these translations. Device registers (MMIO) are also mapped into virtual memory, but with strictly non-cacheable, strongly ordered attributes. If userspace attempts to touch kernel memory or unmapped space, the MMU raises a Data Abort exception, and the kernel terminates the offending process with `SIGSEGV`.
...
- Two-level page tables on ARMv7-A:
...
- Memory attributes:
  ...
  - Device Memory: Non-cacheable, non-bufferable, strictly ordered, side effects on read/write.
...
### 10. Deliberate Fault
Provide a C program that accesses a memory-mapped register address without mapping it first via `mmap()`, treating it as a raw pointer. The program crashes with SIGSEGV. The learner must explain why physical addresses do not exist in the userspace address space and how the MMU enforces isolation.
...
- Transfer to Phase 4: Using `ioremap()` to map physical MMIO addresses into kernel virtual space, and using `copy_to_user()` / `copy_from_user()` to safely transfer data across the privilege boundary.
...
```text
[Pinned Sources: Linux 6.18.y + BusyBox 1.36.1 + Appliance C Source]
                         |
                         v
...
- zImage                            - FHS directory tree
- virt.dtb                          - Static BusyBox binary
...
- Boots QEMU -M virt with earlycon and console capture
...
- **Milestone 1 (Kernel & Device Tree Generation)**: Cross-compile the pinned Linux 6.18.y kernel with the minimal appliance configuration; extract and verify `virt.dtb`.
...
- **Milestone 3 (Automated QEMU Test Harness)**: Create `run_qemu.sh` to boot the appliance in headless mode, capture serial output, verify guest execution of `appliance_diag`, and confirm clean shutdown.
- **Milestone 4 (Fault Injection & Recovery Campaign)**:
  1. *Fault 1 (Bootargs / Storage)*: Inject a corrupted root parameter (`root=/dev/null`). Capture the kernel panic log. Document hypotheses, root cause, and recovery.
  2. *Fault 2 (Userspace Service Dependency)*: Omit `/proc` mounting in the init script. Capture the failure in `appliance_diag`. Document diagnosis, repair, and regression proof.
...
Every serious Embedded Linux debugging task must strictly execute the **7-Step Hypothesis-Driven Diagnostic Loop**:
...
|    - F14: Device MMIO accessed with normal-memory cacheable attributes                |
+---------------------------------------------------------------------------------------+
```
...
- **Hypotheses**: 1. Binary is x86-64 host format; 2. Binary is 64-bit ARM (AArch64) on 32-bit ARM kernel; 3. Binary header is corrupted.
- **Evidence**: `file app` or `readelf -h app`.
- **Root Cause**: Host compiler (`gcc`) was used instead of cross-compiler (`arm-linux-gnueabihf-gcc`).
...
- **Hypotheses**: 1. The binary file does not exist (disproven by `ls`); 2. The dynamic interpreter specified in `PT_INTERP` does not exist in `/lib`; 3. A required shared library in `DT_NEEDED` is missing.
- **Evidence**: `readelf -l app | grep interpreter` reveals `/lib/ld-linux-armhf.so.3`. Checking the rootfs confirms `/lib/ld-linux-armhf.so.3` is absent.
...
- **Hypotheses**: 1. Kernel crashed immediately in `start_kernel()`; 2. Console driver was not compiled into kernel (`CONFIG_SERIAL_AMBA_PL011` missing); 3. Kernel command line specifies the wrong console port (`console=ttyS0` instead of `ttyAMA0`).
- **Evidence**: Add `earlycon=pl011,0x09000000` to `bootargs`. The kernel now prints boot logs, showing `ttyAMA0` is registered, but normal output ceases after earlycon terminates.
...
- **Hypotheses**: 1. `/sbin/init` does not exist; 2. `/sbin/init` lacks executable permissions; 3. `/sbin/init` has an invalid `#!` interpreter line.
- **Evidence**: Kernel boot log shows: `Failed to execute /sbin/init (error -13)`. In Linux error codes, `-13` is `-EACCES` (Permission denied).
- **Root Cause**: Permissions on `rootfs/sbin/init` were set to `644` (`rw-r--r--`) instead of `755` (`rwxr-xr-x`).
...
The Phase 3 Final Gate is a separate, **AI-Free comprehensive transfer assessment** designed to rigorously verify learner capability before advancing to Phase 4 Linux Driver development.

> [!IMPORTANT]
> **Assessment Philosophy**: The Final Gate evaluates **unfamiliar transfer**, not rote replay. Problems use novel parameterizations, unfamiliar binary names, and subtle fault variants. Concrete seeds and answer keys are strictly isolated in `reviewer/` directories.

## 8.1 Gate Structure & Time Contract
- **Duration**: **3.0 hours** strictly timed.
- **Format**: Hands-on practical exam executed on the host system with QEMU.
...
| **Part B** | **Kernel Boot Log, Command-Line & Init Handshake Diagnosis** | **25%** | **60%** | Diagnose a failing serial boot log terminating in a hang or panic; articulate 3 hypotheses; correct command line or init script to achieve clean boot. |
| **Part C** | **Device Tree Resource Inspection & Decompilation** | **25%** | **60%** | Decompile a binary DTB; identify a deliberate resource conflict or disabled status property; patch the DTS, recompile, and verify in QEMU. |
| **Part D** | **Boot-Chain Fault Isolation & Architecture Reasoning** | **30%** | **70%** | Given an integrated appliance image with an obscure multi-stage boot failure, isolate root cause, submit a 7-step postmortem, implement fix, and explain the CPU MMU/privilege mechanism. |
...
3. A fault fix is claimed without providing the formal 7-step postmortem and log evidence;
...
|---|---|---|---|---|
| **Upstream Version Pins** | **VERIFIED** | Official Git repository tags, commit hashes, kernel.org archives | Upstream source existence, tag stability, release dates, licenses | Does not prove local toolchain compatibility without testing |
| **QEMU `virt` Memory Map** | **VERIFIED** | QEMU official documentation (`docs/system/arm/virt.rst`), QEMU source | Hardware register base addresses, IRQ lines, DRAM starting offset | Does not prove physical silicon behavior |
...
| **Final Gate Design** | **UNVERIFIED** | Assessment rubric and exam contract | Rigorous evaluation criteria and floors | Does not prove exam difficulty calibration |
...
- Kernel Kconfig, Build, Image Artifacts  --->   Linux Kernel Engineer: Kernel configuration & patching
...
Phase 3 establishes the precise operational foundation upon which **Phase 4 (Linux Device Drivers & Subsystems)** will execute. 

### What Phase 3 Delivers to Phase 4:
1. **Verified Boot Environment**: A fully functioning, reproducible QEMU ARM Linux system booting Linux 6.18.y LTS with a clean serial console and minimal rootfs.
2. **Kbuild Mental Model**: Total familiarity with `Makefile`, `Kconfig`, `ARCH=arm`, `CROSS_COMPILE=arm-linux-gnueabihf-`, and out-of-tree module compilation mechanics.
3. **Hardware Description Foundation**: Fluent understanding of Device Tree syntax (`compatible`, `reg`, `interrupts`), DTB compilation, and sysfs verification.
4. **Privilege & Memory Boundary**: Thorough comprehension of user virtual address space versus kernel address space, MMU translation, and the necessity of `copy_to_user()` / `copy_from_user()`.
5. **Diagnostic Logging**: Instant ability to read `dmesg`, kernel panics, and oops call traces over serial terminals.

### What Phase 4 Will Build on This Foundation:
- **Module Infrastructure**: Writing `module_init()` / `module_exit()`, module parameters, and licensing.
- **Kernel Driver Model**: `struct bus_type`, `struct device`, `struct device_driver`, `struct platform_device`, and `struct platform_driver`.
- **Driver Probing & DT Matching**: Implementing `of_device_id` tables, `probe()`, `remove()`, and managed resource allocation (`devm_*`).
- **Hardware Register Access**: Using `devm_platform_ioremap_resource()` to map physical MMIO addresses into kernel space, and using `readl()` / `writel()` with appropriate memory barriers.
- **Kernel Concurrency & Interrupts**: Spinlocks, mutexes, wait queues, interrupt handling (`request_threaded_irq()`), top-half versus bottom-half workqueues.
...
- **Scope**:
  ...
  - P3-M02: Download and pin Linux 6.18.y kernel, configure `vexpress_defconfig`, compile `vmlinux` and `zImage`, verify QEMU boot to VFS panic.
...
- **Scope**:
  ...
  - P3-M04: Configure `earlycon` and `console=ttyAMA0`, boot interactive shell in QEMU, trace `dmesg` subsystem milestones.
  - Implement deliberate faults F04 (bad root), F05 (console mismatch), F07 (missing init), F08 (permission denied), and F09 (missing procfs).
- **Acceptance Criteria**: Interactive BusyBox shell booted in QEMU; passing automated tests for all 5 seeded boot faults.
...
- **Scope**:
  ...
  - P3-M06: Configure Buildroot 2026.05.2, create rootfs overlay with custom diagnostic script, build `rootfs.ext4`, boot in QEMU with virtio-blk.
...
- **Scope**:
  - P3-M07: Build memory inspection tools, trace `/proc/<pid>/maps`, demonstrate CPU Data Abort on kernel address dereference, trace `svc` traps.
  - P3-M08: Implement the Reproducible QEMU Embedded Linux Appliance project: top-level Makefile, C diagnostic daemon (`appliance_diag`), automated QEMU launch harness (`run_qemu.sh`), SHA256 manifest.
  - Implement deliberate faults F13 (userspace kernel dereference) and F14 (device MMIO caching).
...
- **Scope**:
  - Author complete AI-Free 4-Part examination harness in `assessment/phase-3-final-gate/`.
  - Provide isolated reviewer semantic oracles and variant seeds in `reviewer/`.
...
- [x] Kernel configuration and build flow (`defconfig`, `.config`, Kbuild).
...
- [x] Shallow reproducible Buildroot build.
...
## Architecture Spine
- [x] User (PL0) vs Kernel (PL1) privilege levels.
- [x] Syscall trap (`svc`) and exception entry.
- [x] Virtual address to physical address translation.
- [x] Two-level page tables (PGD, PTE, TTBR0, TTBR1).
- [x] Cache and TLB maintenance.
- [x] Normal Memory vs Device MMIO attributes.

## Evidence & Diagnostic Discipline
- [x] QEMU virtual platform boot evidence.
...
- [x] Reviewer isolation for answers and seeds.
- [x] Gate implementation deferred to implementation issue.

## Deliverables & Handshake
- [x] `roadmap/phase-3-embedded-linux.md` created.
- [x] `research/phase-3/2026-09-08-embedded-linux-curriculum-design.md` created.
- [x] 5-issue implementation decomposition defined.
- [x] Phase 4 dependency handoff contract established.
