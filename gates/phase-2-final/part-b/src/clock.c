...
bool clock_init(clock_profile_t profile)
{
    ...
    if (profile == CLOCK_PROFILE_72MHZ_HSE) {
        ...
        /* 2 Wait States for Flash at 72 MHz */
        FLASH->ACR = FLASH_ACR_PRFTBE | FLASH_ACR_LATENCY_2;

        /* Bus Dividers: AHB /1, APB2 /1, APB1 /2, ADCPRE /6 */
        ...
        /* HSI / 2 * 16 = 64 MHz, APB1 /2 (32 MHz), ADCPRE /6 */
        RCC->CFGR = RCC_CFGR_HPRE_DIV1   |
                    RCC_CFGR_PPRE2_DIV1  |
                    RCC_CFGR_PPRE1_DIV2  |
                    RCC_CFGR_ADCPRE_DIV6 |
...
void clock_get_frequencies(clock_frequencies_t *freqs)
{
    ...
    freqs->adcclk_hz = freqs->pclk2_hz / adc_div_table[adcpre];
}
