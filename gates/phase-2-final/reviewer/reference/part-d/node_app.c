...
static void task_telemetry(void *pvParameters)
{
    ...
    while (1) {
        ...
        /* High-priority telemetry task acquires shared resource mutex */
...
static void task_storage(void *pvParameters)
{
    ...
    while (1) {
        /* Low-priority storage logging task acquires shared resource mutex */
        if (xSemaphoreTake(xSharedResourceLock, portMAX_DELAY) == pdTRUE) {
            /* With priority inheritance, Task_Storage is elevated to priority 3
             * if Task_Telemetry blocks waiting for the mutex, preventing Task_Compute
             * from preempting it and causing starvation. */
...
{
    /*
     * Initialize shared resource mutual exclusion primitive as a Mutex.
     * xSemaphoreCreateMutex enables FreeRTOS priority inheritance:
     * When a higher-priority task attempts to acquire a mutex held by a lower-priority
     * task, the holder's priority is temporarily boosted to match the waiting task,
     * resolving unbounded priority inversion and avoiding watchdog starvation.
     */
    xSharedResourceLock = xSemaphoreCreateMutex();
    configASSERT(xSharedResourceLock != NULL);
