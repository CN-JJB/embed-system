...
- **Runtime certification**: `scripts/verify_runtime_boot.sh` binds a log to an actual execution (command line, version, console handoff pair, initramfs unpack, init, real BusyBox identity/ps/mounts) and rejects forged, truncated, or mismatched logs.
- **Candidate-binding contract (Round 2)**: the scored candidate is a DATA-ONLY launch manifest (`MACHINE`, `CPU`, `MEM`, `SMP`, `NOGRAPHIC`, `BOOTARGS`). `scripts/parse_candidate_manifest.py` is the single source of truth: it parses the manifest, rejects duplicate/unknown/conflicting/ non-declarative content, validates the canonical values, and constructs the real `qemu-system-arm` argv from those values. `scripts/run_candidate_manifest.sh` records the normalized argv as provenance and executes exactly it; `scripts/verify_candidate_runtime.sh` then re-derives the declarations from the manifest and requires the executed-argv provenance plus the guest console evidence to agree on machine, highmem, GIC, CPU, RAM, SMP, nographic route, kernel, initrd and bootargs. A correct declaration that is not executed, or an execution that overrides the declaration, is REJECTED. Reviewer grading re-executes the submitted manifest through the same trusted runner, so canonical machine/CPU/RAM/SMP values are never substituted for the candidate's own.
