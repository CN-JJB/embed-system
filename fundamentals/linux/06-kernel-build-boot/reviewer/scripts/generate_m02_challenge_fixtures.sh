...
# 1. Generate unfamiliar transfer config with subtle discrepancies
...
# 2. Build candidate_vmlinux matching the 2G/2G memory split
...
"$CC" -nostdlib -T "$OUT_DIR/candidate_vmlinux.lds" -g "$OUT_DIR/candidate_symbols.S" -o "$OUT_DIR/candidate_vmlinux"

# 3. Generate candidate_System.map
"$NM" -n "$OUT_DIR/candidate_vmlinux" | awk '{print $1, $2, $3}' > "$OUT_DIR/candidate_System.map"
