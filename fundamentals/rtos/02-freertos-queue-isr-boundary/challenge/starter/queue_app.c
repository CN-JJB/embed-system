...
static void prvConsumerTask(void *pvParameters)
{
    ...
    for (;;) {
        /* TODO: Block indefinitely on g_sample_queue using xQueueReceive() and portMAX_DELAY */
        if (xQueueReceive(g_sample_queue, &rx_val, /* TODO: block */ 0) == pdPASS) {
            gpio_toggle_pa2();
...
{
    /* TODO: Create queue with length QUEUE_APP_LENGTH and item size QUEUE_APP_ITEM_SIZE */
    g_sample_queue = NULL; /* TODO: xQueueCreate(...) */
    configASSERT(g_sample_queue != NULL);

    /* TODO: Create Consumer task at priority 3 */
...
void timer2_init(uint32_t freq_hz)
{
    ...
    /* TODO: Set Cortex-M3 priority grouping to 0 (all 4 bits pre-emption) */
    /* NVIC_SetPriorityGrouping(0); */

    /* TODO: Set TIM2 priority to CMSIS logical 6 (encoded byte 0x60) */
...
void TIM2_IRQHandler(void)
{
    if ((TIM2->SR & TIM_SR_UIF) != 0) {
        ...
        /* TODO: Enqueue s_seq using xQueueSendFromISR, handle queue-full by incrementing
         * g_isr_dropped_count, and request preemption via portYIELD_FROM_ISR().
         */
    }
}
