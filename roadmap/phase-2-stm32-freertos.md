...
- **Runtime & Startfile Policy:** Standardizes on **Preferred Policy A (newlib-nano runtime with `-nostartfiles` & minimal runtime glue)**. Linker driver passes `-nostartfiles` to explicitly suppress default GCC/newlib CRT startfiles (`crt0.o`, `crti.o`, `crtbegin.o`, `crtend.o`, `crtn.o`), and `-Wl,-e,Reset_Handler` setting the entry point to course `Reset_Handler`. Minimal course runtime glue provides `void _init(void) {}` and `void _fini(void) {}` to satisfy newlib-nano's `__libc_init_array()`. Original assembly startup calls `SystemInit()` (which must not depend on initialized writable global/static C state), copies `.data`, zeroes `.bss`, executes `__libc_init_array()`, and calls `main()`. FreeRTOS `heap_4` is the sole dynamic memory manager in mandatory coursework; libc `malloc/free` is strictly forbidden to prevent dual-heap memory hazards; `_sbrk` is absent when unused; USART telemetry uses direct register I/O without `printf`/host syscall dependency.
...
- **Milestone 0 (Clock, Pinout & Runtime Harness):** Configure 72 MHz SYSCLK via PLL (or 64 MHz HSI fallback if board lacks HSE), set up GPIO timing markers (PA1–PA4), verify Make build with Arm GNU Toolchain 13.3.rel1 (`startup -> SystemInit -> copy .data -> zero .bss -> __libc_init_array -> main`), with `-nostartfiles` suppressing default startfiles, course `Reset_Handler` as sole entry point, minimal `_init/_fini` glue, and verify OpenOCD SWD flashing.
...
- **Runtime & Startfile Contract:** Standardize on **Preferred Policy A (newlib-nano runtime with -nostartfiles, minimal runtime glue & transparent linker)**:
  - Linker driver passes `-nostartfiles` to explicitly suppress default GCC/newlib startfiles (`crt0.o`, `crti.o`, `crtbegin.o`, `crtend.o`, `crtn.o`), preventing any hidden toolchain `_start` or alternative startup code from being linked.
  - Course `Reset_Handler` in `startup_stm32f103xb.s` is the sole entry point, with vector-table offset `+0x04` pointing to `Reset_Handler` and linker driver setting `-Wl,-e,Reset_Handler`.
  - Minimal course runtime glue explicitly provides empty stubs `void _init(void) {}` and `void _fini(void) {}` to satisfy newlib-nano's `__libc_init_array()` and `__libc_fini_array()` without dragging in CRT startup objects.
  ...
  - Linker script explicitly retains `.preinit_array`, `.init_array`, and `.fini_array` using `KEEP` and provides standard `PROVIDE_HIDDEN` boundary symbols (`__preinit_array_start/end`, `__init_array_start/end`, `__fini_array_start/end`).
  - FreeRTOS `heap_4` is the sole dynamic heap (no libc `malloc/free`); `_sbrk` is absent when unused (if present, its pull-in reason must be documented); statistics use integer/fixed-point math (`isqrt()`); direct register USART I/O avoids host syscall/printf dependencies.
  - Link verification contract requires inspecting `arm-none-eabi-gcc -v` to prove startfiles are suppressed, confirming entry point via `readelf -h`, verifying symbols via `readelf -s` / `nm`, and auditing memory bounds in `output.map`.
