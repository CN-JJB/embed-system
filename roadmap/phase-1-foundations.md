...
> Mandatory planned load: **~61–65 h**  
> Weekly planned load: **~8–9.5 h**, preserving substantial unscheduled buffer  
...
| P1-M09 | pthread, Mutex, Race, Lock Misuse | L3 | 6 | race diagnosis + mutex invariant |
| P1-M10 | Linux Systems Telemetry Service Integration | L3 integrated | ~9.5 distributed | project acceptance + fault campaign |
...
3. **one pinned musl syscall wrapper**, preferably `src/unistd/dup2.c` or `src/process/waitpid.c` after Leader/tag verification — libc wrapper vs kernel boundary. Approx. 10–30 LOC.
...
- **M0:** record/API contract + ownership rules.
- **M1:** single-thread parser/codec + tests.
- **M2:** multi-file split + Make + ELF inspection.
- **M3:** FD input + cleanup/leak audit.
- **M4:** one worker + bounded ring + mutex.
- **M5:** SIGINT/SIGTERM graceful shutdown + tracing.
- **M6:** injected malformed record plus one resource/concurrency fault; postmortem + regression.
...
|---|---|---|---|---:|
| 1 | M01 + M02 start; GDB/ASan/UBSan begin | one modern-C companion excerpt | memory/lifetime + FD ownership | 8.5–9 h |
| 2 | M02 finish + M03 + project M0/M2 | CS61C linker material | ELF/link/Make | 9–9.5 h |
| 3 | M04 + project M3 | APUE process comparison | process/zombie/exec | 8–9 h |
| 4 | M05 + M06 + project M1 | signal-safety deeper read | callback/ownership + pipe hang | 9–9.5 h |
| 5 | M07 + M08 | core dump; one static-analyzer trial | binary boundary + unknown bug | 8–9 h |
| 6 | M09 + project M4/M5 | condvar / Unix socket | race/mutex | 9–9.5 h |
| 7 | project M6/final + reconstruction + Final Gate | Tier B source reading | Phase 1 Final Gate | 8.5–10 h |
...
- Keep condition variables **SHOULD** unless the chosen queue implementation genuinely needs them.
