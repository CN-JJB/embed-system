...
- explain the complete Embedded Linux boot sequence on the canonical ARMv7-A virtual platform: `Host Cross-Toolchain -> QEMU Direct Kernel Boot + DTB boot input (r2) -> Self-Extracting Decompressor -> Kernel Entry (stext, MMU off) -> __create_page_tables -> __enable_mmu -> start_kernel()/setup_arch() consumes architecture/DT boot data -> Root Filesystem (initramfs / ext4) -> BusyBox Multi-Call Binary -> PID 1 (/sbin/init) -> Userspace Shell`;
